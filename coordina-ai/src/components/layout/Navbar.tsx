...
export default function Navbar() {
  ...
  // Fetch projects on mount and whenever location changes
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const backendProjects = await projectsApi.list();
        setProjects(backendProjects.map(p => ({
          id: p.id,
          name: p.name,
        })));
      } catch (error) {
          ...
          return (
            <NavLink
              ...
              style={({ isActive: linkActive }) => ({
                ...
                justifyContent: collapsed ? 'center' : 'flex-start',
                borderRadius: 'var(--radius-md)',
                color: linkActive || isActive ? 'var(--white)' : 'var(--grey-600)',
                background: linkActive || isActive ? 'var(--grey-900)' : 'transparent',
                transition: 'all var(--t-fast)',
                fontSize: 13,
                fontWeight: 500,
                cursor: 'pointer',
